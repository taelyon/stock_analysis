...
class MyMainWindow(QMainWindow):
    ...
    def update_stocks(self, nation):
        try:
            db_updater = DBUpdater_etf.DBUpdater()
            if nation == "kr":
    ...
    def update_specific_stock(self):
        try:
            ...
            if company == 'default':
                db_updater.init_db()
                db_updater.update_comp_info('kr')
                db_updater.update_daily_price('kr', 2)
            else:
...
class PortfolioOptimization:
    def __init__(self, stock_list):
        self.mk = DBUpdater_etf.MarketDB()
